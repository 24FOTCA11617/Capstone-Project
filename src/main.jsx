...
function App() {
  ...
  const [wishlist, setWishlist] = useState(() => JSON.parse(localStorage.getItem('northstar-wishlist') || '[]'))
  const [orders, setOrders] = useState(() => JSON.parse(localStorage.getItem('northstar-orders') || '[]'))
  ...
  const updateQuantity = (id, delta) => setCart((current) => current.flatMap((item) => item.id !== id ? item : item.quantity + delta > 0 ? { ...item, quantity: item.quantity + delta } : []))
  const removeFromCart = (id) => setCart((current) => current.filter((item) => item.id !== id))
  const toggleWishlist = (product) => setWishlist((current) => { const next = current.some((item) => item.id === product.id) ? current.filter((item) => item.id !== product.id) : [...current, product]; localStorage.setItem('northstar-wishlist', JSON.stringify(next)); return next })
  const completeOrder = (order) => { setOrders((current) => { const next = [order, ...current]; localStorage.setItem('northstar-orders', JSON.stringify(next)); return next }); setCart([]) }
  return <>
    <Header count={cart.reduce((sum, item) => sum + item.quantity, 0)} onCart={() => setCartOpen(true)} onSearch={() => { setAccountOpen(false); setSearchOpen(true) }} onAccount={() => { setSearchOpen(false); setAccountOpen((current) => !current) }} />
    <main><Routes>
      ...
      <Route path="/shop/:category" element={<Shop addToCart={addToCart} wishlist={wishlist} toggleWishlist={toggleWishlist} />} />
      <Route path="/journal" element={<Journal />} />
      <Route path="/about" element={<About />} />
    </Routes></main>
    <Footer />
    {cartOpen && <CartDrawer cart={cart} onClose={() => setCartOpen(false)} onRemove={removeFromCart} onQuantity={updateQuantity} onOrder={completeOrder} />}
    {searchOpen && <SearchPanel onClose={() => setSearchOpen(false)} />}
    {accountOpen && <AccountPanel user={user} orders={orders} wishlist={wishlist} onUserChange={setUser} onClose={() => setAccountOpen(false)} />}
  </>
}
...
function Header({ count, onCart, onSearch, onAccount }) {
  ...
  return <header className="site-header">
    <div className="announcement"><Sparkles size={13} /> Free shipping on orders over ₹6,300 <span>·</span> Thoughtful goods, simply sent.</div>
    <div className="nav-wrap">
      ...
      <div className="nav-actions">
        ...
        <button className="icon-button account" onClick={onAccount} aria-label="Account"><CircleUserRound size={19} /></button>
...
function ProductCard({ product, addToCart, wishlist, toggleWishlist }) {
  ...
  return <article className="product-card"><div className="product-image"><img src={product.image} alt={product.name} />{product.tag && <span className="product-tag">{product.tag}</span>}<button className={saved ? 'save saved' : 'save'} onClick={() => toggleWishlist(product)} aria-label={saved ? 'Remove from wishlist' : 'Save item'}><Heart size={17} fill={saved ? 'currentColor' : 'none'} /></button><button className={added ? 'quick-add added' : 'quick-add'} onClick={handleAdd}>{added ? <><Check size={14} /> Added to bag</> : <>Add to bag <ArrowRight size={14} /></>}</button></div><div className="product-info"><div><h3>{product.name}</h3><p>{product.category}</p></div><strong>{formatPrice(product.price)}</strong></div></article>
}
...
function SearchPanel({ onClose }) { const [query, setQuery] = useState(''); const [category, setCategory] = useState('All'); const [maxPrice, setMaxPrice] = useState('Any price'); const results = products.filter((product) => `${product.name} ${product.category}`.toLowerCase().includes(query.toLowerCase()) && (category === 'All' || product.category === category) && (maxPrice === 'Any price' || product.price * 84 <= Number(maxPrice))).slice(0, 6); return <div className="panel-backdrop" onClick={onClose}><section className="search-panel" onClick={(event) => event.stopPropagation()}><div className="panel-head"><p className="eyebrow">Find a considered thing</p><button className="icon-button" onClick={onClose} aria-label="Close search"><X size={20} /></button></div><div className="search-input-wrap"><Search size={20} /><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search products" aria-label="Search products" /></div><div className="search-filters"><select value={category} onChange={(event) => setCategory(event.target.value)} aria-label="Filter by category"><option>All</option><option>Carry</option><option>Home</option><option>Wear</option><option>Ritual</option></select><select value={maxPrice} onChange={(event) => setMaxPrice(event.target.value)} aria-label="Filter by maximum price"><option>Any price</option><option value="6300">Under ₹6,300</option><option value="10500">Under ₹10,500</option><option value="16800">Under ₹16,800</option></select></div><div className="search-results">{results.length ? results.map((product) => <Link to="/shop" onClick={onClose} className="search-result" key={product.id}><img src={product.image} alt="" /><span><strong>{product.name}</strong><small>{product.category} · {formatPrice(product.price)}</small></span><ArrowRight size={15} /></Link>) : <p className="no-results">No products found. Try another filter.</p>}</div></section></div> }
function AccountPanel({ user, orders, wishlist, onUserChange, onClose }) { const [mode, setMode] = useState(user ? 'profile' : 'signin'); const [name, setName] = useState(''); const [email, setEmail] = useState(''); const [password, setPassword] = useState(''); const [error, setError] = useState(''); useEffect(() => { setError('') }, [mode]); const submit = (event) => { event.preventDefault(); if (!email.includes('@') || password.length < 6 || (mode === 'signup' && name.trim().length < 2)) { setError(mode === 'signup' ? 'Enter your name, a valid email, and a 6-character password.' : 'Enter a valid email and password of at least 6 characters.'); return } const nextUser = { name: mode === 'signup' ? name.trim() : email.split('@')[0], email }; localStorage.setItem('northstar-user', JSON.stringify(nextUser)); onUserChange(nextUser); setMode('profile') }; const signOut = () => { localStorage.removeItem('northstar-user'); onUserChange(null); setMode('signin') }; return <div className="panel-backdrop" onClick={onClose}><section className="account-panel" onClick={(event) => event.stopPropagation()}><div className="panel-head"><div><p className="eyebrow">Northstar account</p><h2>{mode === 'profile' ? `Hello, ${user.name}.` : mode === 'signup' ? 'Join Northstar.' : 'Welcome back.'}</h2></div><button className="icon-button" onClick={onClose} aria-label="Close account"><X size={20} /></button></div>{mode === 'profile' ? <><p className="account-copy">Signed in as {user.email}.</p><div className="account-stats"><span><strong>{wishlist.length}</strong> saved</span><span><strong>{orders.length}</strong> orders</span></div>{orders.length > 0 && <div className="order-list"><p className="eyebrow">Recent orders</p>{orders.slice(0, 3).map((order) => <div className="order-row" key={order.id}><span>{order.id}<small>{order.date} · {order.items} item(s)</small></span><strong>{formatPrice(order.total)}</strong></div>)}</div>}<button className="button button-dark account-action" onClick={signOut}>Sign out <ArrowRight size={15} /></button></> : <form className="account-form" onSubmit={submit}><input value={name} onChange={(event) => setName(event.target.value)} placeholder="Full name" aria-label="Full name" autoComplete="name" className={mode === 'signup' ? '' : 'hidden-field'} /><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="Email address" aria-label="Email address" autoComplete="email" required /><input type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Password (6+ characters)" aria-label="Password" autoComplete={mode === 'signup' ? 'new-password' : 'current-password'} required />{error && <p className="form-error">{error}</p>}<button className="button button-dark account-action">{mode === 'signup' ? 'Create account' : 'Sign in'} <ArrowRight size={15} /></button><button type="button" className="account-create" onClick={() => setMode(mode === 'signup' ? 'signin' : 'signup')}>{mode === 'signup' ? 'Already have an account? Sign in' : 'New here? Create an account'}</button></form>}</section></div> }
function CartDrawer({ cart, onClose, onRemove, onQuantity, onOrder }) { const [checkoutStarted, setCheckoutStarted] = useState(false); const [address, setAddress] = useState({ name: '', phone: '', line: '', city: '', pincode: '' }); const total = cart.reduce((sum, item) => sum + item.price * item.quantity, 0); const submitOrder = (event) => { event.preventDefault(); onOrder({ id: `NS-${Date.now().toString().slice(-6)}`, date: new Date().toLocaleDateString('en-IN'), total, items: cart.length, address }); setCheckoutStarted('complete') }; return <div className="drawer-backdrop" onClick={onClose}><aside className="cart-drawer" onClick={(event) => event.stopPropagation()}><div className="drawer-head"><h2>{checkoutStarted === true ? 'Delivery details' : checkoutStarted === 'complete' ? 'Order placed' : 'Your bag'} <span>{cart.reduce((sum, item) => sum + item.quantity, 0)}</span></h2><button className="icon-button" onClick={onClose} aria-label="Close bag"><X size={20} /></button></div>{checkoutStarted === 'complete' ? <div className="checkout-message"><Check size={30} /><h3>Thanks for your order.</h3><p>Your order has been placed successfully. We will send updates to your contact details.</p><button className="button button-dark" onClick={onClose}>Continue shopping</button></div> : checkoutStarted === true ? <form className="checkout-form" onSubmit={submitOrder}><input required placeholder="Full name" value={address.name} onChange={(event) => setAddress({ ...address, name: event.target.value })} /><input required placeholder="Phone number" value={address.phone} onChange={(event) => setAddress({ ...address, phone: event.target.value })} /><input required placeholder="Address" value={address.line} onChange={(event) => setAddress({ ...address, line: event.target.value })} /><div className="form-row"><input required placeholder="City" value={address.city} onChange={(event) => setAddress({ ...address, city: event.target.value })} /><input required pattern="[0-9]{6}" placeholder="Pincode" value={address.pincode} onChange={(event) => setAddress({ ...address, pincode: event.target.value })} /></div><div className="cart-total"><div><span>Payable total</span><strong>{formatPrice(total)}</strong></div><button className="button button-dark checkout">Place order <ArrowRight size={15} /></button></div></form> : cart.length === 0 ? <div className="empty-bag"><ShoppingBag size={30} /><p>Your bag is waiting.</p><Link to="/shop" onClick={onClose}>Browse the collection <ArrowRight size={15} /></Link></div> : <><div className="cart-items">{cart.map((item) => <div className="cart-item" key={item.id}><img src={item.image} alt="" /><div><h3>{item.name}</h3><p>{formatPrice(item.price)}</p><div className="quantity-control"><button onClick={() => onQuantity(item.id, -1)} aria-label={`Decrease ${item.name}`}>−</button><span>{item.quantity}</span><button onClick={() => onQuantity(item.id, 1)} aria-label={`Increase ${item.name}`}>+</button></div><button onClick={() => onRemove(item.id)}>Remove</button></div></div>)}</div><div className="cart-total"><div><span>Subtotal</span><strong>{formatPrice(total)}</strong></div><button className="button button-dark checkout" onClick={() => setCheckoutStarted(true)}>Continue to checkout <ArrowRight size={15} /></button><small>Secure demo checkout. Taxes included.</small></div></>}</aside></div> }
